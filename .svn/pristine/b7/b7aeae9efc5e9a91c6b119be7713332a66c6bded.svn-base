    ...
    dispatch_block_t loadImageBlock = ^(void) {
        ...
        if (nil == img && countStamp == [self getTaskCount]) {
            ...
            while (REQUEST_RETRY_COUNT > retryCount && countStamp == [self getTaskCount]) {
                NSLog(@"async load image start url:%@ countStamp:%ld _taskCount:%ld", [request.URL.absoluteString lastPathComponent], (long)countStamp, (long)[self getTaskCount]);
                
                if (0 <= retryCount) {
                    NSLog(@"async load image usleep url:%@ countStamp:%ld _taskCount:%ld", [request.URL.absoluteString lastPathComponent], (long)countStamp, (long)[self getTaskCount]);
                ...
                if (nil == error &&
                    ...
                    NSLog(@"async load image success url:%@ countStamp:%ld _taskCount:%ld dataLength:%lu", [self.imageURL lastPathComponent], (long)countStamp, (long)[self getTaskCount], (unsigned long)[data length]);
                    break;
                } else {
                    data = nil;
                }
                
                NSLog(@"async load image fail url:%@ countStamp:%ld _taskCount:%ld dataLength:%lu", [self.imageURL lastPathComponent], (long)countStamp, (long)[self getTaskCount], (unsigned long)[data length]);
            }
        } else {
            NSLog(@"async load image not start countStamp:%ld _taskCount:%ld", (long)countStamp, (long)[self getTaskCount]);
        }
